# Tracks Rubik's cube's face with Kalman Filter
# Reference: https://github.com/opencv/opencv/blob/4.x/samples/python/plane_tracker.py
...
class planeTracker:
    '''
    Tracks a planar surface using ORB feature extraction and matching
    '''
    def __init__(self):
        '''
        1. Initialized ORB feature detector
        2. Initializes FLANN based matcher
        3. Initializes requried variables 
        '''
    ...
    def track(self, frame):
        '''
        1. Detects ORB features 
        2. Checks if the number of kepoints is beyond a particular threshold
        3. Match it with the features found in the reference image
        4. Shortlist the matches based on each of its second best matches
        5. Sort matches based on the reference image 
        6. Loop for each reference image
            i.   Check if the number of matches id beyond a particular threshold
            ii.  Find homography with the orb feature matches between the reference image and the current frame
            iii. With the corners of the plane from the reference image, find the corresponding corners in the current frame using homography matrix
            iv.  Save the tracked features in a datastructure for further use
        '''
    ...
    def add_target(self, image, rect):
        '''
        Adds referene images
        Detect features on the reference images and save it in a data structure for later use
        '''
        x0, y0, x1, y1 = rect
    ...
    def detect_features(self, frame):
        '''
        Method to detect keypoint and feature descriptors
        '''
...
class VideoPlayer:
    '''
    Initializes camera feed and processes the each frame 
    Checks for the tracked bounding boxes and draws the respective bounding boxes
    '''
    def __init__(self):
    ...
    def play(self):
        '''
        Processes each frame of the camera feed
        '''
        while True:
